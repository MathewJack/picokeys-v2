...
impl<F: Fixed> Wrapping<F> {
    ...
    /// use fixed::Wrapping;
    /// type Wr = Wrapping<I16F16>;
    /// let num = Wrapping(I16F16::from_num(1.5));
    /// assert_eq!(num.unbounded_shl(5), num << 5);
    /// assert_eq!(num.unbounded_shl(32), Wr::ZERO);
    /// ```
    #[must_use = "this returns the result of the operation, without modifying the original"]
    #[inline]
    pub fn unbounded_shl(self, rhs: u32) -> Wrapping<F> {
        Wrapping(self.0.unbounded_shl(rhs))
    }
    ...
    /// assert_eq!(num.unbounded_shr(32), Wr::ZERO);
    /// assert_eq!((-num).unbounded_shr(5), (-num) >> 5);
    /// assert_eq!((-num).unbounded_shr(32), -Wr::DELTA);
    /// ```
    #[must_use = "this returns the result of the operation, without modifying the original"]
    #[inline]
    pub fn unbounded_shr(self, rhs: u32) -> Wrapping<F> {
        Wrapping(self.0.unbounded_shr(rhs))
    }
...
impl<F: FixedSigned> Wrapping<F> {
    ...
    /// use fixed::Wrapping;
    ///
    /// let n = Wrapping(-I16F16::DELTA);
    /// assert_eq!(n.cast_unsigned(), Wrapping(U16F16::MAX));
    /// ```
    #[must_use]
    #[inline]
    pub fn cast_unsigned(self) -> Wrapping<F::Unsigned> {
        Wrapping(self.0.cast_unsigned())
    }
...
impl<F: FixedUnsigned> Wrapping<F> {
    ...
    /// use fixed::Wrapping;
    ///
    /// let n = Wrapping(U16F16::MAX);
    /// assert_eq!(n.cast_signed(), Wrapping(-I16F16::DELTA));
    /// ```
    #[must_use]
    #[inline]
    pub fn cast_signed(self) -> Wrapping<F::Signed> {
        Wrapping(self.0.cast_signed())
    }
