...
use az::{
    Cast, CheckedCast, OverflowingCast, SaturatingCast, StrictCast, UnwrappedCast, WrappingCast,
};
...
macro_rules! cast {
    ($Src:ident($LeEqUSrc:ident); $Dst:ident($LeEqUDst:ident)) => {
        ...
        impl<FracSrc: $LeEqUSrc, FracDst: $LeEqUDst> StrictCast<$Dst<FracDst>> for $Src<FracSrc> {
            #[inline]
            #[track_caller]
            fn strict_cast(self) -> $Dst<FracDst> {
    ...
    ($Fixed:ident($LeEqU:ident); $Dst:ident) => {
        ...
        impl<Frac: $LeEqU> StrictCast<$Dst> for $Fixed<Frac> {
            #[inline]
            #[track_caller]
    ...
    ($Src:ident; $Fixed:ident($LeEqU:ident)) => {
        ...
        impl<Frac: $LeEqU> StrictCast<$Fixed<Frac>> for $Src {
            #[inline]
            #[track_caller]
            fn strict_cast(self) -> $Fixed<Frac> {
